...
def bigloopType1(): # Type 1.0 plays
    ...
    while(currentTime < endTime):
        ...
        if premarket_high < 50:  # changed from $6 to be all inclusive
...
def bigloopType2(): # like type 1.0 plays but i kill at the base of the last known green
    ...
    while(currentTime < endTime):
        ...
        df['24HTime'] = df.apply(lambda row: datetime.fromtimestamp(row.datetime / 1000).strftime('%H%M'), axis=1)
        df['Date'] = df.apply(lambda row: datetime.fromtimestamp(row.datetime / 1000).strftime('%d-%m-%Y'), axis=1)
        df['StopLoss'] = df.apply(lambda row: row.low if row.close > row.open else 1000000, axis=1)
