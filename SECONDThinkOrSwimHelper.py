...
def bigloopType2(): # like type 1.0 plays but i kill at the base of the last known green
    ...
    resultsTable = pd.DataFrame(resultsList,
                                columns=['Instrument', 'BuyPrice', 'BuyTime', "SellTime", "Real%change"])
    ...
    while(currentTime < endTime):
        ...
        multiplier = 1.6  # this is where I'll take profit
        ...
        try:
            ...
            print("for some reason i can't make the dataframe for ", stock)
        ...
        df['StopLoss'] = df.apply(lambda row: row.low if row.close > row.open else False, axis=1)
...
#df['category']=df['age'].apply(lambda x: 'Adult' if x>=18 else 'Child')
       ...
       # print(premarketFri_start)
        ...
        for i in range(len(marketAfterMyBuy)):
            row = marketAfterMyBuy.iloc[i]
            if row['StopLoss'] != False:
    ...
    resultsTable.to_excel(r'C:\Users\rohit kurup\Documents\export_dataframe5.xlsx', index=False, header=True)
